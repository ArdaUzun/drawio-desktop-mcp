...
import { createIpcBridge } from "./mcp/ipc-bridge.js";
...
const silentUpdate =
  !disableUpdate && (process.env.DRAWIO_SILENT_UPDATE === "true" || process.argv.indexOf("--silent-update") !== -1);
...
let isGoogleFontsEnabled =
  store != null ? (store.get("isGoogleFontsEnabled") != null ? store.get("isGoogleFontsEnabled") : false) : false;
...
function isWithinDisplayBounds(pos) {
  ...
  return displays.reduce((result, display) => {
    ...
    return (
      result || (pos.x >= area.x && pos.y >= area.y && pos.x < area.x + area.width && pos.y < area.y + area.height)
    );
  }, false);
}
...
function createWindow(opt = {}) {
  ...
  function rememberWinSize(win) {
    if (store != null) {
      ...
      store.set(
        "lastWinSize",
        size[0] + "," + size[1] + "," + pos[0] + "," + pos[1] + "," + win.isMaximized() + "," + win.isFullScreen(),
      );
    }
  }
...
app.whenReady().then(() => {
  ...
  try {
    ...
    program
      ...
      .option("-g, --page-range <from>..<to>", "selects a page range (1-based, for PDF format only)", argsRange)
      .option("-u, --uncompressed", "Uncompressed XML output (for XML format only)")
      .option("-z, --zoom <zoom>", "scales the application interface", parseFloat)
      .option(
        "--svg-theme <theme>",
      ...
      .option("--mcp-server", "Enable MCP server for AI agent access")
      .option("--mcp-port <port>", "MCP server port (default: 9100)", parseInt)
      .parse(argv);
  } catch (e) {
  ...
  // MCP server configuration — always enabled
  var mcpEnabled = true;
  var mcpPort =
    options.mcpPort || parseInt(process.env.DRAWIO_MCP_PORT || "0") || (store != null && store.get("mcpPort")) || 9100;
  var mcpBridge = null;

  if (mcpEnabled) {
    mcpBridge = createIpcBridge();

    startMcpServer(mcpPort, mcpBridge).catch(function (err) {
    ...
    try {
      ...
      if (options.output) {
        try {
          ...
          //on error, file doesn't exist and it is not a dir
          outType = { isFile: true };
                      ...
                      if (msg == "export-success") {
                        if (data == null || data.length == 0) {
                          ...
                          if (outType != null) {
                            if (outType.isDir) {
                              outFileName =
                                path.join(options.output, path.basename(curFile, path.extname(curFile))) + "." + format;
                            } else {
                              outFileName = options.output;
                            }
                          } else if (inStat.isFile()) {
                            outFileName =
                              path.join(path.dirname(paths[0]), path.basename(paths[0], path.extname(paths[0]))) +
                              "." +
                              format;
                          } //dir
                          else {
                            outFileName =
                              path.join(path.dirname(curFile), path.basename(curFile, path.extname(curFile))) +
                              "." +
                              format;
                          }
                          ...
                          try {
                              ...
                              while (fs.existsSync(realFileName)) {
                                ...
                                realFileName =
                                  path.join(
                                    ...
                                    path.basename(outFileName, path.extname(outFileName)),
                                  ) +
                                  "-" +
                                  counter +
                            ...
                            let fh = fs.openSync(
                              realFileName,
                              fs.constants.O_SYNC | fs.constants.O_CREAT | fs.constants.O_WRONLY | fs.constants.O_TRUNC,
                            );
  ...
  if (!gotTheLock) {
    ...
    app.on("second-instance", (event, commandLine, workingDirectory) => {
      ...
      win.webContents.on("did-finish-load", function () {
        ...
        // Inject MCP plugin into new window
        if (mcpEnabled) {
          try {
            var pluginPath = path.join(__dirname, "mcp-plugin", "mcp-plugin.js");
            var pluginCode = fs.readFileSync(pluginPath, "utf-8");
            win.webContents.executeJavaScript(pluginCode).catch(function (err) {
              console.error("[MCP] Failed to inject plugin:", err);
            });
          } catch (e) {
            console.error("[MCP] Failed to read plugin file:", e);
          }
        }
      });
    });
  ...
  win.webContents.on("did-finish-load", function () {
    ...
    // Inject MCP plugin into renderer
    if (mcpEnabled) {
      try {
        var pluginPath = path.join(__dirname, "mcp-plugin", "mcp-plugin.js");
        var pluginCode = fs.readFileSync(pluginPath, "utf-8");
  ...
  function checkForUpdatesFn(e) {
    if (e != null && e.senderFrame != null && !validateSender(e.senderFrame)) return null;
  ...
  var zoomSteps = [0.25, 0.33, 0.5, 0.67, 0.75, 0.8, 0.9, 1, 1.1, 1.25, 1.5, 1.75, 2, 2.5, 3, 4, 5];
...
async function mergePdfs(pdfFiles, xml) {
  if (pdfFiles.length == 1) {
    ...
    if (xml != null) {
      pdfDoc.setSubject(encodeURIComponent(xml).replace(/\(/g, "\\(").replace(/\)/g, "\\)"));
    }
...
function exportDiagram(event, args, directFinalize) {
  if (event != null && event.senderFrame != null && !validateSender(event.senderFrame)) return null;
...
function checkFileContent(body, enc) {
  if (body != null) {
    ...
    if (c1 == "<") {
      ...
      if (
        (c2 == "i" && c3 == "m" && c4 == "g" && c5 == " ") ||
        (c2 == "i" && c3 == "f" && c4 == "r" && c5 == "a" && c6 == "m" && c7 == "e" && c8 == " ")
      ) {
        return true;
      }
    }
